...
$app->post('/api/detalles-usuario',function(Request $request, Response $response){
    ...
    $ByNombre = $request->getParsedBody()['Busqueda'];
...
$app->put('/api/detalles-usuario/update',function(Request $request, Response $response){
    ...
    $id_detalle_usuario = $request->getParsedBody()['id_detalle_usuario'];
    $nick_user = $request->getParsedBody()['nick_user'];
    $id_rol = $request->getParsedBody()['id_rol'];
    $nombre = $request->getParsedBody()['nombre'];
    $apellido_1 = $request->getParsedBody()['apellido_1'];
    ...
    if($validation->isBlank($id_detalle_usuario) || $validation->isBlank($nick_user) || 
    $validation->isBlank($id_rol) || $validation->isBlank($nombre) || $validation->isBlank($apellido_1) ){
        ...
        echo json_encode($myArray);
            ...
            $sqlExistUsuario1= "SELECT nick_user FROM detalles_usuario where nick_user = '$nick_user' and id_detalle_usuario <> $id_detalle_usuario";
            $resultExisteUsuario1 = $conn->query($sqlExistUsuario1);

            if((mysqli_num_rows($resultExisteUsuario1) != 0) ){

                $myArray[] = array(
                    'status' => $response->getStatusCode(),
                    'msg' => "El nick ya existe"
                );
                echo json_encode($myArray);
            ...
            $sql = "UPDATE detalles_usuario 
            SET nick_user = '$nick_user', id_rol = $id_rol, nombre='$nombre', apellido_1 = '$apellido_1', apellido_2 = '$apellido_2' 
